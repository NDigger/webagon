...
level.onInit = () => {
    ...
    new Lerp(v => level.setOffset(v)).apply(new Vector2(0, 500)).run(new Vector2(0, 0), 7, Lerp.Easing.EASE_IN_OUT)
    new Lerp(v => level.setCenterOffset(v)).apply(new Vector2(0, 500)).run(new Vector2(0, 0), 7, Lerp.Easing.EASE_IN_OUT)
    ...
    const playerRadiusLerp = new Lerp(v => level.setRadius(v));
    const wallScaleLerp = new Lerp(v => level.setWallScale(v));
    timings.forEach((timing, i) => level.createEvent(timing - offset, () => {
        ...
        playerRadiusLerp.apply(50 + t * 20).run(50, t, Utils.easeIn)
        wallScaleLerp.apply(new Vector2(1 + t*.2, 1 + t*.2)).run(new Vector2(1, 1), t, Utils.easeInOut)
    }))
}
