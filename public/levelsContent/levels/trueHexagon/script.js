...
const addPattern = async pKey => {
    if (pKey === 0) await patterns.pInverseBarrage(Utils.mathRandom(2, 3), 400, 300);
    else if (pKey === 1) await patterns.pSpiralBarrage(Utils.mathRandom(4, 6), 200, 300);
    else if (pKey === 2) await patterns.pSpiral(Utils.mathRandom(7, 9), 80, 300, 1);
    else if (pKey === 3) await patterns.pRandomBarrage(Utils.mathRandom(3, 4), 200, 300);
    else if (pKey === 4) await patterns.pTunnel(Utils.mathRandom(2, 3), 400, 300);
}
...
function createEvents(offset) {
    ...
    timings.forEach((timing, i) => level.createEvent(timing - offset, () => {
        ...
        level.setBackgroundRotationOffset(level.getBackgroundRotationOffset() + 360/level.getSides())
        // level.setMainColor(Color.hsvToRgb(hue, 1, .6))
        mainColorLerp.run(Color.hsvToRgb(hue, 1, 1), Color.hsvToRgb(hue, 1, .5), t, Utils.easeOut);
        backgroundFirstColorLerp.run(Color.hsvToRgb(hue, 1., .13), Color.hsvToRgb(hue, 1., .03), t, Utils.easeOut);
        backgroundSecondColorLerp.run(Color.hsvToRgb(hue, 1., .18), Color.hsvToRgb(hue, 1., .05), t, Utils.easeOut);
